...
Uses HuBERT-ECG as frozen feature extractor
Trains only a linear classifier on top
"""
...
TARGET_FS = 500  # HuBERT-ECG uses 500 Hz
TARGET_LEN = 5000  # 10 seconds x 500 Hz
BATCH_SIZE = 16
EPOCHS = 5
LEARNING_RATE = 0.001
MAX_RECORDS = 300
...
def load_ptbxl(path, max_records=MAX_RECORDS):
    ...
    for i, filename in enumerate(selected):
        # Use HR (high resolution) version for 500Hz
...
class LinearProbe(nn.Module):
    ...
    def forward(self, x):
        # x shape: (batch, leads, samples)
        # HuBERT-ECG expects (batch, samples, leads)
        x = x.permute(0, 2, 1)
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    print("\n" + "="*50)
    print(f"Best Macro AUROC (Linear Probe): {best:.4f}")
